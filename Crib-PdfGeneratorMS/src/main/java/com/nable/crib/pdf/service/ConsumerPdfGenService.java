...
import com.nable.crib.pdf.bean.score.ConsumerScoreObject;
import com.nable.crib.pdf.response.PdfResponse;
import com.nable.crib.pdf.util.GetConsumerCribData;

import static org.thymeleaf.templatemode.TemplateMode.HTML;

@Service("ConsumerPdfGenService")
public class ConsumerPdfGenService {
	
	static Logger log = LoggerFactory.getLogger(ConsumerPdfGenService.class);
	
	@Autowired
	GetConsumerCribData getConsCribData;
		
	public PdfResponse get(Long requestDetailId, boolean scoreRequired) throws Exception {
		
		PdfResponse pdfResponse = new PdfResponse();

		ConsumerCribObject consCribObj = getConsCribData.getConsumerCribDataByDetailId(requestDetailId);	
		...
		Context context = getConsumerContext(consCribObj, scoreRequired, consScoreObj);
        String html = loadAndFillTemplate(context, "cons_crib_report_pdf_template");
		...
		pdfResponse.setMessage("Get Consumer CRIB Report PDF Successful");
